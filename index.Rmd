...
notaatiot[nrow(notaatiot)+1,] <- linebreak(c("$p(x), q(x)$", "Tiheysfunktioita"))
notaatiot[nrow(notaatiot)+1,] <- linebreak(c("$\\hat{p}(x), \\hat{q}(x)$", "Tiheysfunktion estimaatteja"))
...
kableExtra::kbl(
  notaatiot, caption = 'Notaatiot', format="latex",
...
lyhsym[2,] <- linebreak(c("SMC", "\\textit{Sequential Monte Carlo}, \nsekventiaalinen Monte Carlo -menetelmä, \nsynonyymi hiukassuotimelle"))
lyhsym[1,] <- linebreak(c("RTSS", "\\textit{Rauch-Turn-Striebel smoother}, Rauch-Turn-Striebel-siloitin"))
lyhsym[3,] <- linebreak(c("BS-PS", "\\textit{Backwards simulation particle smoother},\nTaaksepäin simuloiva hiukassiloitin"))
lyhsym[4,] <- linebreak(c("AoA", "\\textit{Angle-of-Arrival}, Radiosignaalin tulokulman estimointimenetelmä,\njoka perustuu saman signaalin erojen tarkasteluun\nuseamman antennin välillä"))
lyhsym[5,] <- linebreak(c("SIR", "\\textit{Sequential Importance Resampling},\nUudelleenotannan tärkeytyspainoilla suorittava hiukassuodinalgoritmi"))
lyhsym[6,] <- linebreak(c("SIS", "\\textit{Sequential Importance Sampling},\nUudelleenotannan tärkeytyspainoilla suorittava hiukassuodinalgoritmi"))
lyhsym[7,] <- linebreak(c("EKF", "\\textit{Extended Kalman filter}, Laajennettu Kalman-suodin"))
lyhsym[8,] <- linebreak(c("UKF", "\\textit{Unscented Kalman filter}, Hajustamaton Kalman-suodin"))
lyhsym[9,] <- linebreak(c("SLF", "\\textit{Statistically linearized Kalman filter},\nTilastollisesti linearisoitu Kalman-suodin"))
lyhsym[10,] <- linebreak(c("PKF", "\\textit{Position Kalman filter}, Paikkaratkaisun Kalman-suodin"))
lyhsym[11,] <- linebreak(c("MC", "\\textit{Monte Carlo}, Mikä hyvänsä Monte Carlo -menetelmä"))
lyhsym[12,] <- linebreak(c("SNR", "\\textit{Signal-to-noise ratio}, Hyötysignaalin ja kohinasignaalin tehojen suhde"))
lyhsym[13,] <- linebreak(c("ALvar", "\\textit{Adaptive-Lag variance}, Mukautuvan viipeen varianssiestimaatti"))
lyhsym[14,] <- linebreak(c("CLT", "\\textit{Central limit theorem}, Keskeinen raja-arvolause"))
lyhsym[15,] <- linebreak(c("GPS", "\\textit{Global Positioning System}, Yhdysvaltain puolustusministeriön\nkehittämä ja rahoittama satelliittipaikannusjärjestelmä"))
lyhsym[16,] <- linebreak(c("BLE", "\\textit{Bluetooth Low Energy}, Lyhyen matkan langaton likiverkkoteknologia"))
lyhsym[17,] <- linebreak(c("GFSK", "\\textit{Gaussian frequency-shift keying}, Taajuusmodulointimenetelmä"))
lyhsym[18,] <- linebreak(c("OSCU", "\\textit{On-site computing unit}, Pilven ulkopuolella toimiva laskentayksikkö"))
lyhsym[19,] <- linebreak(c("MAC", "\\textit{Medium access control address}, Verkkosovittimen ethernet-verkossa\nyksilöivä osoite"))
lyhsym[20,] <- linebreak(c("IMU", "\\textit{Inertial measurement unit}, Inertiamittausyksikkö"))

lyhsym_sorted <- lyhsym[order(lyhsym[,"Lyhenne"]), ]
kableExtra::kbl(
  lyhsym_sorted, caption = 'Lyhenteet ja symbolit', format="latex",
