...
eli käytetään paikannusvirheenä yksinkertaisesti euklidista etäisyyttä. Tämä antaa paikannusvirheen suoraan metreinä alaluvussa \ref{karttaprojektioista} suoritettavan lineaarisen interpolaation vuoksi. Haluttu, alle metrin paikannustarkkuus saavutetaan, kun missä hyvänsä testiasetelmassa kaikkien asetelmaan liittyvien paikannusvirheiden \(95.\) persentiili on \(\leq\) . Paikannusvirheeseen palataan luvun tulososassa \textbf{TODO}.
...
\emph{angler}-sovellus koostaa jokaisesta ajanhetken \(k\) dataotoksesta atsimuutti- eli suuntimakulman \(\theta_k\) ja korkeuskulman \(\gamma_k\), jotka noudattavat seuraavia jakaumia:
