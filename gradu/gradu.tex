...
Nyt varianssi voidaan laskeat kuten yhtälössä (\ref{CLE-varianssi}) edellä, kun Eeva-indeksit korvataan Henok-indekseillä

\begin{align}\label{OD-sum}
\hat{\mu}_k=\sum_{i=1}^N w_k^i x_k^i[E_{n,k(\lambda)}],
\end{align}
...
Mastrototaro ja Olsson (2023) \citep{Mastrototaro-2023} laajentavat tätä estimaattia edelleen niin, että viive \(\lambda\) valitaan mukautuvasti. Mastrototaron ja Olssonin ALvar-estimaatti (\emph{Adaptive-Lag variance}) lasketaan kuten OD-varianssi edellä, mutta kunkin algoritmin ajokerran jälkeen asetetaan seuraavan ajanhetken \(\lambda\) seuraavasti:
