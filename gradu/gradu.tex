...
Yllä esitetty varianssiestimaatti saattaa kuitenkin kärsiä indeksien ehtymisestä, sillä kun \(k \to \infty\), polveutuvat kaikki hiukkaset lopulta samasta kantaisästä eli indeksit \(E_k^i,\ldots,E_k^N\) ovat kaikki yhtäsuuria. Tämän vuoksi on mielekästä johtaa indeksit ainoastaan tietystä aiemmasta aika-askeleesta alkaen. Olsson ja Douc (2019) \citep{olsson-2019} ehdottavat tähän tarkoitukseen Henok-indeksiä \(E_{m,k}^n\), jossa \(m\) merkitsee aika-askeleen \(m\leq k\) partikkelia, josta kyseinen partikkeli polveutuu. Partikkelien kantaisien sukupolvi määritetään viipeellä \(\lambda\) niin, että \(m=k-\lambda\). Kaavio \ref{fig:henok-indeksit} havainnollistaa partikkelien polveutumista, kun \(\lambda=1\).
...
Nyt varianssi voidaan laskea kuten yhtälössä (\ref{CLE-varianssi}) edellä, kun Eeva-indeksit korvataan Henok-indekseillä. Saadaan painotettu summa
...
\Begin{
  \For{$t=\{1,2,\ldots,T\}$}{
      \Begin{Ajetaan SIR-algoritmi \ref{sir} aika-askeleella $t$\;}
      ...
      \Begin{Jokaiselle $s$ lasketaan varianssiestimaatti $\hat{\sigma}^2_{s|t}$ kuten yhtälössä (\ref{siloitin-varianssi}). Jos $\hat{\sigma}^2_{s|t} < \epsilon$ poistetaan $s$ joukosta $S$ ja käytetään siloitinjaukauman estimaattia $\sum_i^N w_t^i \tilde{x}_{s|t}^i \Omega_t$ kaikille aika-askeleilla $t^\prime \ge t$.\;}
    }
  }  
...
\noindent Empiirisessä esimerkissä jätetään takaisinmuunnos tekemättä, jotta koeympäristön sijaintia ei voi koordinaattien perusteella paikallistaa.
...
Koska esitetty priorijakauma \(p_{x_0}\) on epäinformatiivinen ja monimutkainen koeympäristö aiheuttaa monin paikoin heijastuksia, on mahdollista, että ensimmäisten aika-askeleiden aikana algoritmi tuottaa huonoja sijaintiestimaatteja. Tästä syystä algoritmia ajettaessa suoritetaan ensimmäisellä \(b=3\) aika-askeleella sisäänajo, jonka aikana karttasovitusta ei käytetä ja jonka aikana kohina-arvona käytetään arvoa \(q_{\text{burn\_in}}=q\times10\). Myöskään sijaintiestimaatteja ei sisäänajoaika-askeleilla luoda. Sisäänajon jälkeen sijaintiestimaatit \(\hat{x}_{1:b}\) korvataan ensimmäisellä sisäänajon jälkeisellä sijaintiestimaatilla \(\hat{x}_{b+1}\).
...
\begin{center}\includegraphics{output/figures/unnamed-chunk-127-1} \includegraphics{output/figures/unnamed-chunk-127-2} \includegraphics{output/figures/unnamed-chunk-127-3} \includegraphics{output/figures/unnamed-chunk-127-4} \includegraphics{output/figures/unnamed-chunk-127-5} \includegraphics{output/figures/unnamed-chunk-127-6} \includegraphics{output/figures/unnamed-chunk-127-7} \includegraphics{output/figures/unnamed-chunk-127-8} \includegraphics{output/figures/unnamed-chunk-127-9} \includegraphics{output/figures/unnamed-chunk-127-10} \includegraphics{output/figures/unnamed-chunk-127-11} \includegraphics{output/figures/unnamed-chunk-127-12} \end{center}

\hypertarget{vaihe-2}{%
\section*{Vaihe 2}\label{vaihe-2}}
\addcontentsline{toc}{section}{Vaihe 2}

\begin{center}\includegraphics{output/figures/unnamed-chunk-128-1} \includegraphics{output/figures/unnamed-chunk-128-2} \includegraphics{output/figures/unnamed-chunk-128-3} \includegraphics{output/figures/unnamed-chunk-128-4} \includegraphics{output/figures/unnamed-chunk-128-5} \includegraphics{output/figures/unnamed-chunk-128-6} \includegraphics{output/figures/unnamed-chunk-128-7} \includegraphics{output/figures/unnamed-chunk-128-8} \includegraphics{output/figures/unnamed-chunk-128-9} \includegraphics{output/figures/unnamed-chunk-128-10} \includegraphics{output/figures/unnamed-chunk-128-11} \includegraphics{output/figures/unnamed-chunk-128-12} \includegraphics{output/figures/unnamed-chunk-128-13} \includegraphics{output/figures/unnamed-chunk-128-14} \includegraphics{output/figures/unnamed-chunk-128-15} \includegraphics{output/figures/unnamed-chunk-128-16} \end{center}

\hypertarget{vaihe-3}{%
\section*{Vaihe 3}\label{vaihe-3}}
\addcontentsline{toc}{section}{Vaihe 3}

\begin{center}\includegraphics{output/figures/unnamed-chunk-129-1} \includegraphics{output/figures/unnamed-chunk-129-2} \includegraphics{output/figures/unnamed-chunk-129-3} \includegraphics{output/figures/unnamed-chunk-129-4} \includegraphics{output/figures/unnamed-chunk-129-5} \includegraphics{output/figures/unnamed-chunk-129-6} \includegraphics{output/figures/unnamed-chunk-129-7} \includegraphics{output/figures/unnamed-chunk-129-8} \end{center}
