...
\Begin{
  ...
  \For{$k=\{1,\ldots,T\}$}{
    ...
    } \Else{ \Begin {Sovitetaan partikkeleihin dynaaminen malli (\ref{dynaaminen-malli-empiirinen}) $x_{k+1}=x_k + v_k$, missä $v_k$ on luotu alaluvussa \ref{dynaaminen-malli} esitetyllä menetelmällä sekä kohina-arvolla $q_k\times10$;}}
...
Ohessa esitetyn algoritmin suoritusnopeus on perusmuodossaan luokkaa $\mathcal{O}(N)$. Varianssin estimoinnissa käytetään ALvar-varianssiestimaattia (kts. yhtälöt (\ref{CLE-varianssi}) ja (\ref{ALvar-lambda}). Taulukossa \ref{tab:wb-parametrit} on esitetty algoritmin suunnitteluparametrit, joiden valintaan palataan empiirisessä esimerkissä alla.
...
Algoritmin R-koodi löytyy osoitteesta https://github.com/rintakumpu/progradu/analyysi/R/pf_positioning.R.
