...
missä $v_k$ on kohinaa. Kohina luodaan jokaiselle ajanhetkelle seuraavasti. First the desired random walk movement distance d is sampled from the truncated Normal distribution with a=0, b=q_max and sd=q where q_max and q are adjustable parameters. Then the direction to which to move p is sampled from Uniform distribution with a=0 and b=2pi. Now the noise vector v_k is created as v_k = (cos(p)* d, sin(p)*d). 
