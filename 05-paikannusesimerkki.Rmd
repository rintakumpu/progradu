...
Paikannusvirheen laskemisessa on etäisyysfunktiona $d(\cdot)$ käytetty `raster`-kirjaston `pointDistance()`-funktiota. Koodissa on korostettu tiiviyden sijaan luettavuutta ja koodi on kommentoitu kattavasti. Algoritmin R-koodi löytyy https://github.com/rintakumpu/progradu/analyysi, mistä löytyy myös asetelmassa käytetty data csv-muodossa sekä empiirisen esimerkin muun koodin sisältävä R Markdown -notebook.
...
Data on kuvattu tarkemmin alaluvussa TODO. Koeympäristön pohjapiirustus on esitetty kuvassa \ref{fig:pohjapiirustus}. Piirustuksessa XR-2-paikantimet on kuvattu punaisilla numeroiduilla ympyröillä. Kuljettu testipolku on merkitty violetilla janalla. 
...
\noindent Dataa kerättiin sekä liikkuvalla että paikallaan olevalla testilaitteella. Paikallaan oleva testilaite asetettiin pöydälle 1.5 metrin korkeudelle. Liikkuvassa tapauksessa testilaite kiinnitettiin robottiin, joka oli ohjelmoitu seuraamaan haluttua liikerataa vastaavia lattiamerkintöjä ennalta määrätyllä vakionopeudella. Esitetyt mallit soveltuvat kuitenkin myös liikkuvalle laitteelle. Data kerättiin aikaan, jolloin tavaratalon käyttöaste oli alhainen. Tällä minimoitiin radiosignaalien tielle osuvien ihmisten vaikutus signaaleihin.

\subsection{Parametrien valinta} \label{parametrien-valinta}

Paikannus suoritettiin kolmessa vaiheessa. Kussakin vaiheessa paikannusalgoritmi ajettiin jokaisella vaiheeseen liittyvällä suunnitteluparametrikombinaatiolla $r=30$ kertaa ja tulokset (paikannusvirhe, suoritusaika, varianssi) laskettiin näiden $30$ ajon aritmeettisena keskiarvona.
...
Kaavion TODO paikannuspoluissa ei ole piirretty karttasovituksessa käytettyjä liikeen sisätilapolygoneja. Polygonit on jätetty piirtämättä, jotta tavaratalon tunnistaminen olisi vaikeampaa.
